            ...
            <div class="mb-16">
                ...
                <div class="relative overflow-hidden">
                    <div id="recommendedCarousel" class="flex gap-5 transition-transform duration-500 ease-out will-change-transform">
                        @foreach($recommendedSeries as $recSeries)
                        <div class="flex-shrink-0 w-[calc(50%-10px)] sm:w-[calc(33.33%-13.33px)] md:w-[calc(25%-15px)] lg:w-[calc(20%-16px)] group cursor-pointer" onclick="window.location.href='{{ route('web-series.show', $recSeries->id) }}'">
                            <div class="relative rounded-xl overflow-hidden bg-gray-900 hover:shadow-2xl transition-all duration-300 group-hover:scale-105">
                                <div class="relative w-full aspect-video bg-gradient-to-br from-purple-600/30 to-pink-600/30 flex items-center justify-center overflow-hidden">
                                    @if($recSeries->cover_image)
                                        <img src="{{ asset($recSeries->cover_image) }}" alt="{{ $recSeries->project_name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    @else
                                        <i class="fas fa-tv text-white/30 text-5xl"></i>
                                    @endif
                                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                    @if($recSeries->status === 'completed')
                                    ...
                                    @else
                                    <div class="absolute top-2 left-2">
                                        <span class="bg-purple-600 text-white text-xs px-2 py-0.5 rounded-full">{{ ucfirst($recSeries->status) }}</span>
                                    </div>
                                    @endif
                                    <div class="absolute bottom-2 right-2 bg-black/50 backdrop-blur-sm rounded-full px-2 py-0.5 flex items-center gap-1">
                                        ...
                                        <span class="text-white text-xs">{{ $recSeries->views_count ?? rand(100, 999) }}</span>
                                    </div>
                                </div>
                                <div class="p-3">
                                    <h4 class="text-white font-semibold text-sm truncate">{{ $recSeries->project_name }}</h4>
                                    <div class="flex items-center justify-between mt-1">
                                        <p class="text-gray-400 text-xs">{{ $recSeries->episodes_count ?? $recSeries->episodes->count() }} episodes</p>
                                        <div class="flex items-center gap-1">
                                            <i class="fas fa-star text-yellow-400 text-xs"></i>
                                            <span class="text-gray-300 text-xs">{{ number_format($recSeries->rating ?? rand(4, 5), 1) }}</span>
                                        </div>
                                    </div>
                                    @if($recSeries->category)
                                    <div class="mt-2">
                                        <span class="text-xs bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full">{{ $recSeries->category->name }}</span>
                                    </div>
                                    @endif
                                </div>
                            </div>
