...
<div class="min-h-screen bg-black py-[120px] px-4">
    <div class="container mx-auto max-w-7xl">
        ...
        <div class="mb-8">
            @if(isset($episode))
            <a href="{{ route('web-series.episodes', $series->id) }}" class="text-gray-400 hover:text-white transition-colors inline-flex items-center gap-2 group">
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center gap-3 mb-2">
                <span class="px-3 py-1 rounded-full text-sm font-bold bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg">
                    Episode {{ $episode->episode_number }}
                </span>
                <h1 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">{{ $episode->title }}</h1>
            </div>
            <div class="flex gap-3 mt-2">
                ...
                <span class="text-gray-500 text-xs">{{ $totalScenes }} Segments</span>
            </div>
            @else
        ...
        @php
            if(isset($episode)) {
                ...
                $scenesCollection = $episode->scenes;
            } else {
                $totalScenes = $series->scenes->count();
                $completedVideos = $series->scenes->whereNotNull('video_url')->count();
                $allVideosCompleted = $totalScenes > 0 && $completedVideos == $totalScenes;
                $scenesCollection = $series->scenes;
            }
        @endphp
        
        <div class="mb-8 flex justify-between items-center">
            <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full {{ $allVideosCompleted ? 'bg-blue-500' : 'bg-purple-500' }} animate-pulse"></div>
                <span class="text-xs text-gray-400" id="completedCount">{{ $completedVideos }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($scenesCollection as $index => $scene)
            @php
                ...
                if($index > 0) {
                    $previousScene = $scenesCollection[$index - 1];
                ...
                <div class="bg-gradient-to-br from-gray-900/80 to-gray-800/40 rounded-xl border border-gray-700/50 overflow-hidden transition-all duration-500">
                    ...
                    <div class="px-4 py-3 border-b border-gray-700/50 bg-gradient-to-r from-gray-800/50 to-gray-900/50">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold segment-icon 
                                    {{ $isVideoCompleted ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white' : 
                                       ($isImageCompleted ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white' : 'bg-gray-700 text-gray-500') }}">
                                    @if($isVideoCompleted)
                            ...
                            <div>
                                <span class="text-[10px] px-2 py-1 rounded-full flex items-center gap-1 segment-status
                                    {{ $isVideoCompleted ? 'bg-green-500/20 text-green-400' : 
                                       ($isImageCompleted ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-700 text-gray-500') }}">
                                    @if($isVideoCompleted)
                    ...
                    <div class="p-3 segment-content">
                        ...
                        @elseif($isImageCompleted && !$isVideoCompleted)
                            <!-- Generate Button -->
                        ...
                        @elseif(!$isImageCompleted && $isFirstSegment)
    ...
    <div class="bg-gradient-to-br from-gray-900/80 to-gray-800/40 rounded-xl border border-gray-700/50 overflow-hidden transition-all duration-500 hover:border-purple-500/50 hover:shadow-xl hover:shadow-purple-500/10 transform hover:-translate-y-1">
        
        <div class="px-4 py-3 border-b border-gray-700/50 bg-gradient-to-r from-gray-800/50 to-gray-900/50">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold episode-preview-icon bg-gray-700 text-gray-500 transition-all duration-300" id="episodePreviewIcon">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                        </svg>
                    </div>
                ...
                <div>
                    <span class="text-[10px] px-2 py-1 rounded-full bg-gray-700 text-gray-500 flex items-center gap-1 episode-preview-status transition-all duration-300" id="episodePreviewStatus">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                        </svg>
                        <span class="episode-preview-status-text" id="episodePreviewStatusText">Locked</span>
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="p-3">
            <div class="relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-lg border border-gray-700 overflow-hidden episode-preview-content transition-all duration-300 group-hover:border-purple-500/30" style="aspect-ratio: 4 / 3;" id="episodePreviewContent">
                <div class="relative z-10 flex flex-col items-center justify-center h-full text-center p-4">
                    <div class="w-16 h-16 mx-auto mb-3 rounded-full bg-gray-800 flex items-center justify-center transition-all duration-500 episode-icon-container group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-purple-500/20" id="episodeIconContainer">
                        <svg class="w-6 h-6 text-gray-600 episode-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" id="episodeIcon">
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
</svg>
                    </div>
                    <p class="text-gray-400 text-sm font-medium episode-preview-locked-text transition-all duration-300 group-hover:text-gray-300" id="episodePreviewLockedText">Episode Preview Locked</p>
                    ...
                    <div class="mt-3 w-full max-w-[180px] mx-auto">
                        <div class="w-full bg-gray-700 rounded-full h-1.5 overflow-hidden">
                            <div class="bg-gradient-to-r from-purple-500 to-pink-500 h-1.5 rounded-full transition-all duration-500 transform origin-left" style="width: 0%" id="episodeProgress"></div>
                        </div>
                    </div>
                    ...
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none" id="pulseRing" style="display: none;">
                        <div class="w-32 h-32 rounded-full border-2 border-purple-500/30 animate-ping"></div>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="relative">
                                    ...
                                    <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none">
                                        <svg class="w-4 h-4 text-purple-400 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="mb-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" id="genderSwitch" class="sr-only peer" onchange="filterVoicesByGender()">
...
let activeVideoGenerations = {};
let videoPollingIntervals = {};
...
function showToast(message, type = 'success') {
    ...
    if (!toast) return;
    const colors = { success: 'bg-green-600', error: 'bg-red-600', info: 'bg-blue-600', warning: 'bg-purple-600' };
...
function updateSegmentWithVideo(sceneId, videoUrl) {
    console.log('Updating segment with video:', sceneId, videoUrl);
    ...
    const imgUrl = sceneCard.getAttribute('data-image-url');
    ...
    contentContainer.innerHTML = `
        <div class="relative group/video bg-black rounded-lg overflow-hidden cursor-pointer" style="aspect-ratio: 4 / 3;">
            <div class="absolute inset-0 w-full h-full overflow-hidden">
                <img src="${imgUrl}" class="w-full h-full object-cover blur-2xl scale-110 opacity-60">
            </div>
            <div class="absolute inset-0 bg-black/30"></div>
    ...
    if (statusSpan) {
        statusSpan.className = 'text-[10px] px-2 py-1 rounded-full bg-green-500/20 text-green-400 flex items-center gap-1';
    ...
    if (iconDiv) {
        iconDiv.className = 'w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/20';
    ...
    if (dot) {
        dot.classList.remove('bg-gray-700', 'bg-purple-500');
        dot.classList.add('bg-green-500');
    }
    
    updateCompletedCount();
    unlockNextSegment(sceneId);
    
    showToast('✅ Video ready! Click play to watch.', 'success');
}

function unlockNextSegment(sceneId) {
    const currentCard = document.getElementById(`scene-${sceneId}`);
    if (!currentCard) return;
    
    const nextCard = currentCard.nextElementSibling;
    if (nextCard && nextCard.classList.contains('segment-card')) {
        const lockOverlay = nextCard.querySelector('.locked-overlay');
        if (lockOverlay) {
            lockOverlay.style.display = 'none';
            showToast(`🔓 Segment ${nextCard.getAttribute('data-scene-number')} is now unlocked!`, 'success');
        }
    }
}

function startVideoPolling(sceneId) {
    if (videoPollingIntervals[sceneId]) clearInterval(videoPollingIntervals[sceneId]);
    
    let attempts = 0;
    const maxAttempts = 90;
    
    videoPollingIntervals[sceneId] = setInterval(async () => {
        attempts++;
        
        try {
            const response = await fetch(`/api/scene/${sceneId}/video-status`, {
                headers: {
            ...
            if (data.video_url && data.video_url !== '') {
                clearInterval(videoPollingIntervals[sceneId]);
                delete videoPollingIntervals[sceneId];
                activeVideoGenerations[sceneId] = false;
                delete activeVideoGenerations[sceneId];
                updateSegmentWithVideo(sceneId, data.video_url);
                ...
                activeVideoGenerations[sceneId] = false;
                delete activeVideoGenerations[sceneId];
                showToast('⏰ Video generation is taking longer than expected. Check back later.', 'warning');
            }
        } catch (error) {
...
function startVideoGeneration(sceneId) {
    ...
    if (activeVideoGenerations[sceneId]) {
        showToast('⏳ Video is already generating in background...', 'info');
        return;
    }
    ...
    const contentContainer = sceneCard.querySelector('.segment-content');
    const existingVideoUrl = sceneCard.getAttribute('data-video-url');
    ...
    if (existingVideoUrl && existingVideoUrl !== '') {
        showToast('✅ Video already generated!', 'success');
        unlockNextSegment(sceneId);
        return;
    }
    
    activeVideoGenerations[sceneId] = true;
    
    const loadingMessages = ["🖼️ Analyzing image...", "🎬 Generating video with AI...", "✨ Adding effects...", "🎥 Finalizing..."];
    let messageIndex = 0;
    
    contentContainer.innerHTML = `
    <div class="relative rounded-lg overflow-hidden border border-purple-500/30" style="aspect-ratio: 4 / 3;">
        <div class="absolute inset-0 w-full h-full overflow-hidden"><img src="${imageUrl}" class="w-full h-full object-cover blur-2xl scale-110 opacity-40"></div>
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center z-10 p-4 text-center">
            ...
            <p class="text-purple-400 text-sm font-semibold" id="loadingMsg-${sceneId}">${loadingMessages[0]}</p>
            <p class="text-gray-400 text-xs mt-2">This may take 30-60 seconds</p>
        </div>
    </div>
`;
    ...
    if (statusSpan) {
        statusSpan.className = 'text-[10px] px-2 py-1 rounded-full bg-purple-500/20 text-purple-400 flex items-center gap-1';
        statusSpan.innerHTML = '<svg class="w-3 h-3 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Generating Video';
    }
    
    if (iconDiv) {
        iconDiv.className = 'w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-purple-500/20 text-purple-400';
        iconDiv.innerHTML = '<svg class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';
    }
    ...
    (async () => {
        try {
            ...
            if (result.success && result.video_url) {
                clearInterval(messageInterval);
                updateSegmentWithVideo(sceneId, result.video_url);
                showToast('✅ Video generated successfully!', 'success');
                activeVideoGenerations[sceneId] = false;
                delete activeVideoGenerations[sceneId];
            } else if (result.success) {
                showToast('🎬 Video generation started! This may take 30-60 seconds.', 'success');
...
// ==================== EPISODE PREVIEW CARD FUNCTIONS ====================
...
function updateCompletedCount() {
    ...
    console.log(`updateCompletedCount: ${completedCount}/${totalSegments}`);
...
function updateEpisodePreviewCard() {
    ...
    console.log(`updateEpisodePreviewCard: ${completedCount}/${totalSegments}, allCompleted: ${allCompleted}`);
    
    const previewIcon = document.getElementById('episodePreviewIcon');
    const previewStatus = document.getElementById('episodePreviewStatus');
    const previewStatusText = document.getElementById('episodePreviewStatusText');
    const previewContent = document.getElementById('episodePreviewContent');
    const previewLockedText = document.getElementById('episodePreviewLockedText');
    ...
    const iconContainer = document.getElementById('episodeIconContainer');
    const episodeIcon = document.getElementById('episodeIcon');
    ...
    if (allCompleted) {
        if (previewIcon) {
            ...
            previewIcon.innerHTML = '<svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>';
        }
        ...
        if (previewStatusText) previewStatusText.innerHTML = 'Watch Now';
        ...
        if (episodeIcon) {
            episodeIcon.innerHTML = '<svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>';
        }
        if (pulseRing) {
        ...
        if (!window.episodeReadyToastShown) {
            window.episodeReadyToastShown = true;
            showToast('🎉 All segments completed! Episode is ready to watch!', 'success');
        }
        
    } else if (completedCount > 0) {
        // IN PROGRESS STATE - Using purple/pink instead of yellow
        if (previewIcon) {
            previewIcon.className = 'w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/20';
            previewIcon.innerHTML = '<svg class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';
        }
        if (previewStatus) {
            previewStatus.className = 'text-[10px] px-2 py-1 rounded-full bg-purple-500/20 text-purple-400 flex items-center gap-1';
        }
        if (previewStatusText) previewStatusText.innerHTML = 'In Progress';
        if (previewLockedText) {
            previewLockedText.innerHTML = '🎬 Generating your episode...';
            previewLockedText.className = 'text-purple-400 text-sm font-semibold';
        }
        if (episodeProgressMessage) {
            episodeProgressMessage.innerHTML = `${completedCount} of ${totalSegments} segments completed`;
            episodeProgressMessage.className = 'text-purple-400 text-xs mt-2';
        }
        if (previewContent) {
        ...
        if (iconContainer) {
            iconContainer.className = 'w-16 h-16 mx-auto mb-3 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center transition-all duration-500';
        }
        if (episodeIcon) {
            episodeIcon.innerHTML = '<svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>';
        }
        ...
        if (previewStatusText) previewStatusText.innerHTML = 'Locked';
        if (previewLockedText) {
        ...
        if (episodeIcon) {
            episodeIcon.innerHTML = '<svg class="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>';
        }
        window.episodeReadyToastShown = false;
    }
}
...
function collectEpisodeVideos() {
    ...
    document.querySelectorAll('.segment-card').forEach(card => {
        ...
        const isCompleted = videoUrl && videoUrl !== '';
...
function getAssetUrl(path) {
    ...
    // If it starts with /, use as is (relative to domain root)
    if (path.startsWith('/')) {
        return path;
    }
...
// Alternative: If you need the full domain URL (Laravel asset equivalent)
...
function loadThumbnails() {
    ...
    episodeVideoUrls.forEach((video, index) => {
        ...
        thumbnailCard.className = `w-full rounded-lg overflow-hidden border transition-all duration-300 cursor-pointer border-purple-500/50 hover:border-purple-500`;
        ...
        thumbnailCard.innerHTML = `
            <div class="relative aspect-video bg-gray-800">
                ${video.imageUrl && video.imageUrl !== '' ? 
                    `<img src="${video.imageUrl}" class="w-full h-full object-cover blur-sm" alt="Segment ${video.sceneNumber}">` :
            ...
            <div class="p-1 text-center bg-gray-800/50">
                <span class="text-xs text-gray-400">${video.sceneNumber}</span>
            </div>
        `;
    ...
    if (playlistCount) {
        playlistCount.textContent = `${episodeVideoUrls.length} segments`;
    }
}
...
function playVideoAtIndex(index) {
    ...
    const video = episodeVideoUrls[index];
    player.src = video.url;
    player.load();
    player.play().catch(e => console.log('Play error:', e));
    
    if (currentSegmentInfo) {
        currentSegmentInfo.innerHTML = `Segment ${video.sceneNumber} of ${episodeVideoUrls.length}`;
    }
    
    document.querySelectorAll('#thumbnailContainer > div').forEach((thumb, i) => {
        if (i === index) {
            thumb.classList.add('ring-2', 'ring-purple-500', 'scale-105');
            thumb.classList.remove('opacity-70');
        } else {
            thumb.classList.remove('ring-2', 'ring-purple-500', 'scale-105');
...
function playPrevious() {
    if (currentPlaylistIndex > 0) {
        ...
        showToast('This is the first segment', 'info');
    }
}
...
function playNext() {
    if (currentPlaylistIndex < episodeVideoUrls.length - 1) {
        ...
        showToast('This is the last segment', 'info');
    }
}
...
function updateVideoTime() {
    ...
    const episodeProgressPercent = document.getElementById('episodeProgressPercent');
    ...
    if (video && video.duration && !isNaN(video.duration)) {
        ...
        if (videoTimeInfo) {
            videoTimeInfo.innerHTML = `${currentMin.toString().padStart(2, '0')}:${currentSec.toString().padStart(2, '0')} / ${totalMin.toString().padStart(2, '0')}:${totalSec.toString().padStart(2, '0')}`;
        }
        
        const progress = (video.currentTime / video.duration) * 100;
        if (episodeProgressBar) {
            episodeProgressBar.style.width = `${progress}%`;
        }
        if (episodeProgressPercent) {
            episodeProgressPercent.innerHTML = `${Math.round(progress)}%`;
        }
    }
}
...
function openEpisodeModal() {
    ...
    console.log('Found videos:', episodeVideoUrls.length);
    ...
    const totalSegments = document.querySelectorAll('.segment-card').length;
    const completedCount = episodeVideoUrls.length;
...
function toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
        modal.requestFullscreen().catch(err => console.log('Fullscreen error:', err));
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Page loaded, checking for existing videos...');
